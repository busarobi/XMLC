...
public class LomTree extends MLLRFHR {
	
	private static final long serialVersionUID = 4576116358399421362L;

	PointerBasedDecisionTree tree = null;
	
	private static Logger logger = LoggerFactory.getLogger(LomTree.class);
	...
	public LomTree(Properties properties, StepFunction stepfunction) {
		super(properties, stepfunction);

		System.out.println("#####################################################" );
		System.out.println("#### Learner: LomTree" );
		System.out.println("#####################################################" );
	...
	public void allocateClassifiers(AVTable data) {
		...
		this.tree = new PointerBasedDecisionTree((int) (this.treeSize));
	...
	public void train(AVTable data) {
		...
		for (int ep = 0; ep < this.epochs; ep++) {
			...
			for(int i = 0; i < traindata.n; i++) {
				...
				DecisionTreeNode node = this.tree.getRoot();
				...
				while(!end) {
					...
					if(!node.isLeaf()) {
						double response;
						if(node.getExpectedScore() >= node.getExpectedScore(label)) {
							response = -1;
						}	else {
							response = 1;
						}
	...
	protected void updatedPosteriors( int currIdx, DecisionTreeNode node, double inc) {
	...
	public double getPartialPosteriors(AVPair[] x, DecisionTreeNode node) {
	...
	public DecisionTreeNode findLeaf(AVPair[] x) {
	...
	public double estimateProbability(AVPair[] x, int label) {
		DecisionTreeNode node = this.findLeaf(x);
