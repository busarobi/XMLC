...
import util.OfflineTree1;

public class PLTFHOfflineTree1 extends MLLRFH {
	private static final long serialVersionUID = 7616508564698690408L;

	private static Logger logger = LoggerFactory.getLogger(PLTFHOfflineTree1.class);

	transient protected int t = 0;
	
	transient OfflineTree1 tree = null;
	...
	protected Object readResolve(){
		this.tree = new OfflineTree1(this.treeFileName);
		this.t = this.tree.getSize(); 
		this.fh = FeatureHasherFactory.createFeatureHasher(this.hasher, fhseed, this.hd, this.t);
		return this;
	}
	
	public PLTFHOfflineTree1(Properties properties, StepFunction stepfunction) {
		super(properties, stepfunction);
		
		logger.info("#####################################################" );
		logger.info("#### Learner: PLTFTHOfflineTree1" );
		
		// k-ary tree
		this.k = Integer.parseInt(this.properties.getProperty("k", "2"));
		this.treeFileName = this.properties.getProperty("TreeFile", "");
