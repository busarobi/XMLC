...
public class OnlinePLTmacroF {
	...
	public OnlinePLTmacroF(AVTable data) {
		this.traindata = data;
		this.m = data.m;
		this.d = data.d;
		this.t = 2 * this.m - 1;
		...
		this.tp = new double[this.m];
		this.y = new double[this.m];
		this.haty = new double[this.m];
	...
	public void train() {
		...
		for (int ep = 0; ep < this.epochs; ep++) {
			...
			for (int i = 0; i < traindata.n; i++) {
				...
				HashSet<Integer> predictedLabels = this.getPositiveLabels(traindata.x[currIdx]);
				...
				for (int j = 0; j < traindata.y[currIdx].length; j++) {
					...
					int treeIndex = label + traindata.m - 1;
					...
					while(treeIndex > 0) {
						...
						//posterior *= this.getPartialPosteriors(traindata.x[currIdx], treeIndex);
					...
					this.thresholds[label + traindata.m - 1] = this.tp[label] / (this.haty[label] + this.y[label]);
				...
				for(int predictedLabel : predictedLabels) {
					...
					this.thresholds[predictedLabel + traindata.m - 1] = this.tp[predictedLabel] / (this.haty[predictedLabel] + this.y[predictedLabel]);
	...
	public double getPosteriors(AVPair[] x, int label) {
		...
		double posterior = getPartialPosteriors(x, treeIndex);
...
//		String testfileName = "../data/mediamill/test-exp1.svm";
		...
		OnlinePLTmacroF learner = new OnlinePLTmacroF(data);
