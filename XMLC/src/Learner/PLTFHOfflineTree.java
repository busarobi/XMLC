...
import util.OfflineTree;

public class PLTFHOfflineTree extends MLLRFH {
	private static final long serialVersionUID = 7616508564698690408L;

	private static Logger logger = LoggerFactory.getLogger(PLTFHOfflineTree.class);

	transient protected int t = 0;
	
	transient OfflineTree tree = null;
	...
	protected Object readResolve(){
		this.tree = new OfflineTree(this.treeFileName);
		this.t = this.tree.getSize(); 
		this.fh = FeatureHasherFactory.createFeatureHasher(this.hasher, fhseed, this.hd, this.t);
		return this;
	}
	
	public PLTFHOfflineTree(Properties properties, StepFunction stepfunction) {
		super(properties, stepfunction);
		
		logger.info("#####################################################" );
		logger.info("#### Learner: PLTFTHOfflineTree" );
